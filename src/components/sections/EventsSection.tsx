...
import { Trophy, BookOpen, Calendar, Clock, Sparkles, Star } from "lucide-react";
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10" ref={ref}>
        ...
        >
          ...
          <div className="absolute left-6 lg:left-1/2 top-0 bottom-0 w-px lg:-translate-x-px">
            <motion.div
              initial={{ scaleY: 0 }}
              whileInView={{ scaleY: 1 }} viewport={{ once: true, margin: "-50px" }}
          ...
          {/* View More <Star size={14} className="inline ml-2" /> button logic (only if not showAll) */}
          {!showAll && currentEvents.length > 5 && visibleCount < currentEvents.length && (
            <div className="flex flex-col items-center mt-10 gap-4">
              ...
              >
                View More <Star size={14} className="inline ml-2" />
              </a>
            </div>
          )}
